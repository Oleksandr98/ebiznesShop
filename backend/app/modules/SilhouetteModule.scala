...
import com.mohiva.play.silhouette.impl.providers.oauth2.{DropboxProvider, FacebookProvider, GoogleProvider}
...
import models.repository.{OAuthRepository, PasswordRepository, UserRepository}
...
class SilhouetteModule extends AbstractModule with ScalaModule {
  ...
  def provideSocialProviderRegistry(googleProvider: GoogleProvider,
                                    dropboxProvider: CustomDropboxProvider): SocialProviderRegistry = {
    SocialProviderRegistry(Seq(googleProvider, dropboxProvider))
  }
  ...
  @Provides
  def provideDropboxProvider(httpLayer: HTTPLayer,
                             socialStateHandler: SocialStateHandler,
                             configuration: Configuration): CustomDropboxProvider = {
    new CustomDropboxProvider(httpLayer, socialStateHandler, configuration.underlying.as[OAuth2Settings]("silhouette.dropbox"))
  }
  ...
  def oAuth2InfoDao(dbConfigProvider: DatabaseConfigProvider): DelegableAuthInfoDAO[OAuth2Info] =
    new OAuthRepository(dbConfigProvider)
