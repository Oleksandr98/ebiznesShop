...
class CardController @Inject()(val crdRepo: CardRepository, custRepo: CustomerRepository,
  ...
  def addCardJSON() = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def closeCard(id: Long): Action[AnyContent] = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def closeCardForm(id: Long): Action[AnyContent] = Action.async { implicit request: Request[AnyContent] =>
    crdRepo.closeById(id).map {
      case 1 => Redirect(CardsUrl + id + "/form")
      case 0 => BadRequest(views.html.businesserror(s"Not found item by id: $id"))
    }
  }

  def blockCard(id: Long): Action[AnyContent] = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def modifyCardJSON(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
    cardUpdateForm.bindFromRequest.fold(errorForm => {
      Future.successful(BadRequest(Json.obj(
        "status" -> "Error",
        "details" -> Json.toJson(errorForm.errors)
