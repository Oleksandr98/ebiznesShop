...
class LocationController @Inject()(val lcnRepo: LocationRepository,
  ...
  final val unknownErrorMsg = "Unknown Error"
  ...
  def addLocationJSON() = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
    locationAddForm.bindFromRequest.fold(errorForm => {
      ...
      { locData =>
          try {
            ...
            case _ => Future.successful(InternalServerError(unknownErrorMsg))
          }
      })
  }
    ...
    locationAddForm.bindFromRequest.fold(errorForm => {
      ...
      { locData =>
          try {
            ...
            case _ => Future.successful(InternalServerError(views.html.businesserror(unknownErrorMsg)))
          }
      })
  }
  ...
  def modifyLocationJSON(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def removeLocation(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
