...
class OrderController @Inject()(val ordRepo: OrderRepository, ctmRepo: CustomerRepository,
                                prdRepo: ProductRepository, scc: DefaultSilhouetteControllerComponents)
  ...
  def getOrders() = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
    for {
      list <- ordRepo.getAll()
    } yield {
  ...
  def getOrder(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def createOrderJSON() = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
