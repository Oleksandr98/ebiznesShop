...
class ProductController @Inject()(val prdRepo: ProductRepository, catRepo: CategoryRepository,
  ...
  def addProductJSON = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>

    var categ: Seq[CategoryData] = Seq[CategoryData]()
    catRepo.getAll().onComplete {
  ...
  def updateProductJSON(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
    productUpdateForm.bindFromRequest.fold(errorForm => {
      Future.successful(BadRequest(Json.obj(
        "status" -> "Error",
        "details" -> Json.toJson(errorForm.errors)
  ...
  def removeProduct(id: Long): Action[AnyContent] = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
