...
@Singleton
class CategoryController @Inject()(val catRepo: CategoryRepository, scc: DefaultSilhouetteControllerComponents)
  ...
  def addCategoryJSON = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def removeCategoryJSON(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
    catRepo.removeById(id).map {
