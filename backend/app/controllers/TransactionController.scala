...
class TransactionController @Inject()(val tranRepo: TransactionRepository, ctmRepo: CustomerRepository, lcnRepo: LocationRepository,
                                      ordRepo: OrderRepository, scc: DefaultSilhouetteControllerComponents) extends SilhouetteController(scc) {
  ...
  def getTransactions() = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
    for {
  ...
  def getTransaction(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
    tranRepo.getById(id).map {
  ...
  def modifyTransactionJSON(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def createSaleTransactionJSON() = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def reverseTransaction(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
