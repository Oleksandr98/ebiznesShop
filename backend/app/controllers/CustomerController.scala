...
@Singleton
class CustomerController @Inject()(val scc: DefaultSilhouetteControllerComponents, ctmRepo: CustomerRepository)
  extends SilhouetteController(scc) {

  final val customersUrl = "/customers/"
  ...
  def getCustomers() = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
    for {
      list <- ctmRepo.getAll()
    } yield {
      Ok(Json.toJson(list)).withHeaders(("Access-Control-Allow-Credentials", "true"))
    }
  }

  def getCustomer(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def modifyCustomerJSON(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def modifyCustomer(id: Long) = Action.async { implicit request =>
    customerUpdateForm.bindFromRequest.fold(errorForm => {
      ...
      ctmRepo.updateById(id, p).map {
        case 1 => Redirect(customersUrl + id + "/form")
  ...
  def modifyCustomerForm(id: Long) = Action.async { implicit request =>
    ...
    cust.map {
      case Some(x) =>
        val prdForm = customerUpdateForm.fill(WSUpdateCustomerData(Option.apply(x.name), Option.apply(x.surname), Option.apply(""),
  ...
  def blockCustomer(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def blockCustomerForm(id: Long) = Action.async { implicit request =>
    ctmRepo.blockOrUnblockById(id, true).map {
      case 1 => Redirect(customersUrl + id + "/form")
      case 0 => BadRequest(views.html.businesserror(s"Not found item by id: $id"))
    }
  ...
  def closeCustomer(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def unblockCustomer(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
