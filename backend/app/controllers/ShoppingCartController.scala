...
class ShoppingCartController @Inject()(val shcRepo: ShoppingCartRepository, ctmRepo: CustomerRepository, ordRepo: OrderRepository,
                                       ...
                                       scc: DefaultSilhouetteControllerComponents) extends SilhouetteController(scc) {

  final val cartUrl = "/shopping-cart/"
  ...
  def createCartJSON() = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def placeOrder(id: Long, cId: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def createCart() = Action.async { implicit request =>
    shoppingCartAddForm.bindFromRequest.fold(errorForm => {
      ...
      shcRepo.create(shcData).map(id => Redirect(cartUrl + id + "/form"))
    })
  }
  ...
  def getCart(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def addToCartJSON(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def addToCart(id: Long) = Action.async { implicit request =>
    shoppingCartProdAddForm.bindFromRequest.fold(errorForm => {
      ...
      shcRepo.addProduct(id, data.prdId, data.quantity).flatMap(x => x.map {
        case 1 => Redirect(cartUrl + id + "/form")
  ...
  def removeFromCartJSON(id: Long, prodId: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def modifyCartJSON(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
  ...
  def removeCart(id: Long) = securedAction.async { implicit request: SecuredRequest[EnvType, AnyContent] =>
